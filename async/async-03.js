 ...
 * await라는 키워드가 뒤쪽에 있는 비동기 함수의 실행을 기다린다는 의미를 가지고 있다.
 * 단, await라는 키워드를 사용하려면, 반드시 분명하게 async를 사용하는 함수안에서 작성되어야 한다는 것이다.
 * async와 await은 꼭 세트로 같이 사용해야 한다.
 *
 * 주의할 점은 await라는 키워드는 console.log()와 같은 단순히 값을 출력하는 동기 함수 등에 붙여서 사용하면 안된다.
 * console.log()는 Promise 인스턴스를 반환하지 않기 떄문입니다.
