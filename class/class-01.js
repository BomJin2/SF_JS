...
const fruits = new Array("사과", "바나나", "체리"); // new키워드를 통해 새로운 인스턴스를 만든다.
 ...
 * 여기서는 fruits라는 배열데이터 혹은 인스턴스라고 일컫는다.
 * 이렇게 인스턴스에서 쓸 수 있는 별도의 속성이나 메서드를 등록하는 객체
 *
 * 우리가 원시 타입, 참조 타입들도 마찬가지로 다양하나 데이터들은 생성자 함수를 통해서 반환된 결과이고
 * 우리가 그동안 생성자 함수를 사용하지 않고, 각각의 데이터를 만든 것은 더 쉽게 만들 수 있게 각각의 리터럴 방식으로 만들었을 뿐
 */
...
const user = {
   ...
   * 속성뒤에 있는 콜론(:) 기호와 function 키워드를 생략할 수 있다고도 배웠음
   */
};
...
/** 객체 데이터의 메서드를 일반 함수로 만들면, 일반 함수에서의 this는 함수가 호출되는 곳에서 정의된다고 배웠음
 * 즉, user가 this가 됩니다
 */
 ...
 *
 * 1번 방식을 따르면, 정확하게 같은 로직을 가지고 있는 함수가 2번 만들어 져서 굉장히 비효율적
 *
 * 2. user.getFullName.call(user2)
 */

console.log(user.getFullName.call(user2)); // => 재활용하는 코드의 가독성도 떨어지고, 메서드를 빌려쓰는 구조에서 본래의 객체도 참조해야하고 여러 어려움이 있음

/** 이러한 단점을 보완하기 위해 우리가 쓸 수 있는 것이 바로 프로토타입 입니다. */
